---
name: query-config
description: Answer questions about the dotfiles configuration and the current machine's state — what tier it's on, what's tracked vs untracked, what's installed vs missing, what a given config does, where files live. Use when the user asks descriptive/inspection questions ("what tier am I on?", "what's tracked under ~/.config/git/?", "is pv installed?", "where do my SSH keys come from?").
---
...
3. **`doctor`** — convenient roll-up of OS, package manager, tier,
...
|---|---|
| What OS / pkg-mgr / tier? | `. ~/.dotfiles/scripts/os-detect.sh; env \| grep DOTFILES_` |
| What tier is persisted? | `cat ~/.config/dotfiles/system-type 2>/dev/null \|\| echo "(unset / common-only)"` |
| Overall health | `JUST_JUSTFILE="$HOME/.dotfiles/Justfile" just doctor` |
| What's tracked under a path? | `git --git-dir="$HOME/.dotfiles.git" --work-tree="$HOME" ls-tree --full-tree -r --name-only HEAD -- <path>` |
| Is file X tracked? | `git --git-dir="$HOME/.dotfiles.git" --work-tree="$HOME" ls-files --error-unmatch <path>` (exit 0 = yes) |
...
| What packages should be installed on this tier? | `cat ~/.dotfiles/packages/common.txt ~/.dotfiles/packages/$(cat ~/.config/dotfiles/system-type 2>/dev/null).txt 2>/dev/null` |
