...
- The actual bootstrap runs as `su - USER -c '...'` over the *same* root SSH connection — not a second `ssh` hop — since a freshly created account has no password or authorized key of its own to log in with yet.
