...
**From an already-bootstrapped machine, `dj setup [user@]host [extra install.sh args...]` does all of this for you** — it starts (or reuses) a local ssh-agent, loads this machine's shared `~/.ssh/id_ed25519`, then runs install.sh on the target over `ssh -A -t` with this machine's own `--private-repo` remote and `--sops [user-of-this-machine]@[this-machine]` filled in automatically, so the forwarded agent covers both the private-repo clone and the age-key scp back to this machine (same shared identity, already authorized on both ends per §5.3). Extra args (`--system-type`, `--theme`, ...) pass through unchanged:
