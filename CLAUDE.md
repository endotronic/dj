...
**`dj root-setup hostname [extra install.sh args...]`** — for a machine you have root on but no account of your own on yet (a shared/multi-user box, or one you're joining as a second admin). It connects as `root@hostname`, checks whether a user named after *this* machine's own invoking user (`id -un`) already exists there, and either creates one or reuses the existing one — then bootstraps this project into that account's home directory exactly as `dj setup` would if pointed at it directly:

```sh
dj root-setup lobos --system-type desktop --theme '#2596be'
```

- **New account:** the target's own package manager is detected remotely (apt vs pacman genuinely differ here — Debian/Ubuntu's admin group is `sudo`, Arch's is `wheel`) and `useradd --skel /dev/null` creates a *fresh, empty* home directory — deliberately skipping `/etc/skel`, since install.sh (run next, as this new user) populates it instead. A narrow, temporary `/etc/sudoers.d` NOPASSWD rule scoped to just this username is granted for the duration of the bootstrap (a brand-new account has no password to answer sudo's own prompt with) and removed again at the end, success or failure.
- **Existing account:** nothing is created or touched — install.sh runs in that account's real home directory (read via `getent passwd`, not assumed to be `/home/<user>`) with its own normal conflict-aware checkout, same as pointing `dj setup` at it.
